...
impl Default for StorageBudgetConfig {
    fn default() -> Self {
        Self {
            ...
            pressure_floor_messages_per_channel: 800,
...
mod tests {
    ...
    fn checked_in_configuration_example_parses() {
        ...
        assert_eq!(config.block_store.max_messages_per_channel, 5_000);

        let documented: Config =
            toml::from_slice(include_bytes!("../config/example.toml")).unwrap();
        assert_eq!(documented.block_store.max_messages_per_channel, 5_000);
